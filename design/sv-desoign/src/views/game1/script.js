// Game state
  ...
  function generateRandomMap() {
    ...
    rooms.forEach(room => {
      ...
      newMap[doorY] = newMap[doorY].substring(0, doorX) + "D" + newMap[doorY].substring(doorX + 1);
    });
    ...
    rooms.forEach(room => {
      ...
      for (let y = room.y + 1; y < room.y + room.height - 1; y++) {
        for (let x = room.x + 1; x < room.x + room.width - 1; x++) {
          ...
          if (Math.random() < 0.3) {
            let symbol = encounterSymbols[Math.floor(Math.random() * encounterSymbols.length)];
    ...
    switch (symbol) {
      ...
      case "C":
        addActionButton("Open Chest", () => {
          generateText("Choose what you find in the chest: Gold, Potion or Armor.").then((loot) => {
      ...
      case "L":
        addActionButton("Collect Loot", () => {
          selectLoot().then((selectedLoot) => {
            addToInventory(selectedLoot, "Valuable treasure found during your adventure.");
      ...
      case "?":
        addActionButton("Discover...", async () => {
          let encounterDetails = await generateText("Choose an encounter: Monster, NPC, or find Loot.");
