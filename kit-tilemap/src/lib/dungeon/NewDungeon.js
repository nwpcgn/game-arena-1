...
export const NewDungeon = (settings) => {
	const Dungeon = {
		...
		split(node) {
			if (
				node.level.length > this.maxRoomSize ||
				node.level[0].length > this.maxRoomSize
			) {
				...
				if (splitDirection === VERTICAL) {
					...
					rNode.level = node.level.map((row) =>
						row.slice(indexToSplit, row.length)
					)
				} else {
