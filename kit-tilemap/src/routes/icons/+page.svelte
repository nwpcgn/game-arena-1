<script lang="ts">
	...
	import { getTile, groupArray, checkCollide, distance } from '$lib/index.js'
	let frame = $state({
		width: 0,
		height: 0
	})
	...
	function movePlayer(x: number, y: number) {
		const newX = game.position.x + dx
		const newY = game.position.y + dy
...
<section class="page-layer nwp">
	<article
		class="content"
		style="--size: var(--page-width);"
		bind:clientWidth={frame.width}
		bind:clientHeight={frame.height}>
		<div class="rogue-grid bg-neutral" style={gridStyle}>
